...
const tokenABNBParAddress = process.env.PAIR_ADDRESS// Address of token B's BNB pool
const bnbAddress = process.env.BNB_ADDRESS;
const tokenAAddress = process.env.TOKENA_ADDRESS; // Address of token A
const amountToSell = web3.utils.toWei('1', 'ether'); // Amount of token B to sell
const minAmountA = web3.utils.toWei('0.5', 'ether'); // Minimum amount of token A to receive

const exchangeContract = new web3.eth.Contract(exchangeABI, tokenABNBParAddress);
